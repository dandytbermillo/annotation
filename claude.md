...
- Persistence: **PostgreSQL (primary)**, **IndexedDB (fallback for offline)**
...
- Must support **Web (Next.js)** and **Electron** builds.
  - Web: prefer Postgres via API layer for server deployments; keep IndexedDB fallback for offline.
  - Electron: may use local Postgres or SQLite for local persistence; adapters must abstract this choice.
